...
function Content ({ roomName, setRoomName, saveLayout, onClose }: Props): JSX.Element {
  return (
    <VStack w='280px' h='160px'>
      ...
      <VStack w='280px' h='130px' bg={themeColor.main} rounded='3px' paddingX='5px'>
        <RoomForm initialValue={roomName} setName={setRoomName} />
        <Box onClick={onClose}>
...
function SaveField ({ roomName, setRoomName, saveLayout }: Props): JSX.Element {
  return (
    <>
      <Box display={{ base: 'none', '2xl': 'block' }}>
        <Content roomName={roomName} setRoomName={setRoomName} saveLayout={saveLayout}/>
      </Box>
      <Popover>
        <PopoverTrigger>
