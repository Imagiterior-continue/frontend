...
function IconLink ({ link, icon, text }: Props): JSX.Element {
  return (
    <>
      <HStack paddingX='30px' borderLeftWidth='3px' borderColor={themeColor.accentString} cursor='pointer' transition='.1s' _hover={{ bg: themeColor.accentHover }} onClick={() => { window.location.href = link }}>
        {icon}
